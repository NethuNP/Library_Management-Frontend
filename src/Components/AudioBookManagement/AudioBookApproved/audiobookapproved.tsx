...
import AudioBookapprovedModel from "./audiobookapprovedModel";
...
const AudioBookapproved = () => {
  ...
  const books =
    responseData?.books.filter(
      (book) => book.status === "Approved" && book.format === "Audio"
    ) ?? [];
...
export default AudioBookapproved;
