...
import VedioBookapprovedModel from "./videobookapprovedModel";
...
const VideoBookapproved = () => {
  ...
  const books =
    responseData?.books.filter(
      (book) => book.status === "Approved" && book.format === "Video"
    ) ?? [];
...
export default VideoBookapproved;
