...
import AudioBookrejectedModel from "./audiobookrejectedModel";
...
const AudioBookrejected = () => {
  ...
  const rejectedBooks =
    responseData?.books.filter(
      (book) => book.status === "Rejected" && book.format === "Audio"
    ) ?? [];
...
export default AudioBookrejected;
