...
import videobook from "@/Components/VideoBooks/videobook";
import Image from "next/image";
import BookDetailModal from "./bookdetailModel";

const Publishervideobook = () => {
  const [selectedVideoBook, setSelectedVideoBook] = useState<any | null>(null);

  const handleBookClick = (book: any) => {
    setSelectedVideoBook(book);
  };
  ...
  return (
    <div>
      ...
      {selectedVideoBook && (
        <BookDetailModal book={selectedVideoBook} onClose={handleCloseModal} />
      )}
    </div>
  );
};
