...
import VideoBookpendingModel from "./videobookpendingModel";
...
const VideoBookpending = () => {
  ...
  // Filter books that are pending
  const books =
    responseData?.books.filter(
      (book) => book.status === "Pending" && book.format === "Video"
    ) ?? [];

  const [deleteBook, { isLoading: deleteLoading }] = useDeleteBookMutation();

  const handleDelete = async (id: string) => {
    // Show a SweetAlert confirmation before deleting
...
export default VideoBookpending;
